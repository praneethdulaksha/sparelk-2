...
export default function RatingsReviews({ item }: Props) {
  ...
  return (
    reviews && (
      <>
        <h2 className="px-5 mt-16 text-2xl md:text-4xl">
              ...
              {ratings.map((_, i) => {
                return (
                  <RatingBar
                    ...
                    stars={5 - i}
                    count={ratings[4 - i]}
                    totReviewsCount={reviews.length}
                  />
                );
              })}
            </div>
            <div className="flex flex-col items-end md:1/2 lg:w-3/5">
              <label className="text-5xl md:text-7xl lg:text-8xl">
                {item.rating}
              </label>
                ...
                {Array(1, 2, 3, 4, 5).map((n, i) => {
                  return (
                    <FontAwesomeIcon
                      ...
                      className="mr-0 text-black"
                    />
                  );
                })}
              </span>
              <label className="text-zinc-500 text-base md:text-lg lg:text-xl">
